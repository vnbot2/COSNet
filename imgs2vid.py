...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_counts', type=int, default=int(10e12))
    ...
    if args.mode == 'v2i':
        ...
        def getFrame(sec):
            ...
            if hasFrames:
                ...
                out_file = os.path.join(output_dir, f'{count}.jpg')
                print(out_file)
                cv2.imwrite(out_file, image)     # save frame as JPG file
            return hasFrames

        sec = 0
        frameRate = 1/args.fps #//it will capture image in each 0.5 second
        count=1
        success = getFrame(sec)
        while success:
            count = count + 1
            if count > args.max_counts:
                break
            sec = sec + frameRate
            sec = round(sec, 2)
            success = getFrame(sec)
